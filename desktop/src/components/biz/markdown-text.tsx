// 答案要点/追问的 markdown 渲染面;铺满所在容器,行长由外层列宽控制。
...
export function MarkdownText({ text, className }: { text: string; className?: string }) {
  ...
  return (
    <div
      className={cn(
        'text-sm leading-relaxed [&_a]:text-primary [&_code]:rounded [&_code]:bg-muted [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-[13px] [&_li]:ml-4 [&_li]:list-disc [&_ol_li]:list-decimal [&_p]:my-1.5 [&_p:first-child]:mt-0 [&_strong]:font-semibold',
        className,
      )}
