// shadcn/ui Command(官方源码,基于 cmdk;⌘K 面板唯一容器)
...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      <DialogContent className="overflow-hidden p-0 top-[20%] translate-y-0 sm:max-w-xl" showClose={false}>
        <DialogTitle className="sr-only">{title}</DialogTitle>
        <Command className="[&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:text-muted-foreground [&_[cmdk-group-heading]]:text-xs [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group]]:px-2 [&_[cmdk-input-wrapper]_svg]:size-4 [&_[cmdk-input]]:h-12 [&_[cmdk-item]]:px-2 [&_[cmdk-item]]:py-2.5">
          {props.children}
        </Command>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div data-slot="command-input-wrapper" className="flex h-12 items-center gap-2 border-b px-3">
      ...
      <CommandPrimitive.Input
        ...
        className={cn(
          'flex h-12 w-full bg-transparent text-sm outline-hidden placeholder:text-muted-foreground/70 disabled:opacity-50',
          className,
        )}
        {...props}
      />
