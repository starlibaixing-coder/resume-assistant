...
async function fillQuestionForm(page: Page, title: string, focus: string, answer: string) {
  ...
  await page.getByLabel('答案要点').fill(answer);
}
...
test('添加题目页:手动保存 → 直接进我的题库(不经待审核),来源标手动', async ({ page }) => {
  ...
  await page.getByLabel('标签').fill('js, 手写');
...
test('侧栏「我的题库」落地页即有添加题目入口', async ({ page }) => {
  ...
  // 加完即入队:保存后落题库空间;计数与入口可见
  await expect(page).toHaveURL(/library\?category=my/);
  await page.goto('/#/my');
  await expect(page.getByText('共 1 题')).toBeVisible({ timeout: 5_000 });
  await expect(page.getByRole('link', { name: '添加题目' }).first()).toBeVisible();
});
