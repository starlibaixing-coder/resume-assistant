...
async function fillQuestionForm(page: Page, title: string, focus: string, answer: string) {
  await page.getByLabel('题干').fill(title);
  await page.getByLabel('考察点(focus)').fill(focus);
  await page.getByLabel('答案要点(一行一条,合计 ≥50 字)').fill(answer);
}

test('深链手动加题 → 直接进我的题库(不经草稿区)', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '加入我的题库', exact: true }).click();

  // toast + 题出现在我的题库浏览列表(无草稿区步骤)
...
test('侧栏「我的题库」落地队列页即有手动加题入口', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: '手动加题' })).toBeVisible();
});

test('手动加题可进既有模块;校验失败拦截保存', async ({ page }) => {
  await page.goto('/#/generate?mode=manual');
  await expect(page.getByRole('tab', { name: '手动加题' })).toHaveAttribute('data-state', 'active');
  await page.getByLabel('新模块名').fill('面试手写');
  ...
  // 第二题:模块下拉已自动指到刚建的模块(01 · 面试手写),校验失败拦截
