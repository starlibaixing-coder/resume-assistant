...
>
> **2026-08-27 追加修复(用户反馈"看不到手动加题入口"):** D6 的入口只放在浏览页,而侧栏「我的题库」落地在队列页,空库时用户在落地页找不到入口。已补:队列页空态 + 头部双入口(与浏览页同款);首页空卡片改链到 `/my` 并更新文案。追加 e2e「落地队列页即有手动加题入口」,全量 16 e2e + 196 单测复绿。
>
> **施工中新增的关键修正(供后人):**
