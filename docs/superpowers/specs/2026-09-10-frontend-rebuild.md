...
| 日期 | 变更 |
|---|---|
| 2026-09-15(三) | 服务商预设 Ollama 本地 → 自定义(任意 OpenAI 兼容服务:URL/模型必填、Key 可选,`knownProvider` 把已移除预设的存值归空回落默认);生成参数区拍平重排(去组标签套弱化标签的双层结构,四参数与 Base URL/模型同级、标准字段标签,「留空用官方默认」与各家推荐沉到底部一行提示)——修复用户反馈「表单丑」 |
