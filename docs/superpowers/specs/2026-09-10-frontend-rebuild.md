...
| 页面 | 语法 | 要点 |
|---|---|---|
| 今日 | 驾驶舱纵排 | 问候+日期;**主计划卡 hero**(第一件未完成事 = 唯一主 CTA:到期优先复习,其次学习,截断 batch_size);7 日到期预测柱(hover 显当日数);题库概览四卡;最近学习记录。零值行不渲染;全清走 EmptyState+「总复习」 |
...
| 题库 | 双栏工作台(54%列表+详情) | 分类 Tabs(官方分类名+我的);筛选工具栏:状态/难度/来源(仅我的)/模块各一个 Select(默认项「全部…」,全部 h-9 同一排左对齐流动)+搜索输入;有激活筛选时出现「重置」;行=状态药丸+题干+模块+难度,选中朱砂左条;>200 行窗口化虚拟滚动(行高 41px);详情=完整题面+来源行(source_ref 口径)+笔记+动作(练习这一题/编辑就地/删除 AlertDialog/官方题可复制入库);深链 `?cat= ?qid= ?status=`,↑/↓ 移动选中,分类记忆 `meta.last_bank_cat` |
...
- **⌘K 命令面板**(cmdk):动作(开始复习/开始学习 N/总复习/去审核,与今日页按钮同名同义,无「混合」类型)+ 前往(八路由)+ 题目(全库字段包含式过滤,选中落 `?qid=` 详情)+ 按 JD 生成。
