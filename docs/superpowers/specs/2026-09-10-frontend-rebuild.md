...
| 题库 | 双栏工作台(54%列表+详情) | 分类 Tabs(官方分类名+我的);筛选:状态五档按钮组+难度+来源(仅我的)+模块下拉+关键词;行=状态药丸+题干+模块+难度,选中朱砂左条;>200 行窗口化虚拟滚动(行高 41px);详情=完整题面+来源行(source_ref 口径)+笔记+动作(练习这一题/编辑就地/删除 AlertDialog/官方题可复制入库);深链 `?cat= ?qid= ?status=`,↑/↓ 移动选中,分类记忆 `meta.last_bank_cat` |
