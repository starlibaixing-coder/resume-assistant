...
| 日期 | 变更 |
|---|---|
| 2026-09-15 | AI Key 按服务商分存(用户反馈:只配了 DeepSeek 别家也显示已配置):secrets 名改为 `llm-api-key:<provider>`,启动一次性迁移把旧全局 Key 挂到当时激活的服务商并清除旧条目(注意:迁移按 meta.ll_provider 归属,若当时字段与 Key 实际归属不符需手工重贴);Ollama 本地无需 Key——ready/引导/测试连接均放行空 Key;切换服务商即清空 Key 输入框防误存到别家 |
