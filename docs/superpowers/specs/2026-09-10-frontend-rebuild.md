...
一句话:**暖纸书页上放一张冷静的工作台骨架**。内容区是"纸"(米纸底、墨字、宋体标题),管理面是"工作台"(列表+详情双栏、筛选芯片、就地编辑)。视觉上与旧版(56px 图标栏+顶部工具条)形神俱变,第一眼可辨。
...
- **壳 = 文字侧栏(216px)+ 无顶栏 + 底部状态栏**:
  - 侧栏:朱砂方章品牌位;【研习】【求职】两组;审核项带待审数药丸;当前项朱砂浅底;hover 显 ⌘N 快捷键。
...
| 页面 | 语法 | 要点 |
|---|---|---|
| 今日 | 驾驶舱纵排 | 问候+日期;**主计划卡 hero**(第一件未完成事 = 唯一主 CTA:到期优先复习,其次学习,截断 batch_size);7 日到期预测柱(hover 显当日数);题库概览四卡;最近学习记录。零值行不渲染;全清走 EmptyState+「再过一遍」 |
...
| 题库 | 双栏工作台(54%列表+详情) | 分类 Tabs(官方分类名+我的);筛选:状态五档芯片+难度+来源(仅我的)+模块下拉+关键词;行=状态药丸+题干+模块+难度,选中朱砂左条;>200 行窗口化虚拟滚动(行高 41px);详情=完整题面+来源行(source_ref 口径)+笔记+动作(练习这一题/编辑就地/删除 AlertDialog/官方题可复制入库);深链 `?cat= ?qid= ?status=`,↑/↓ 移动选中,分类记忆 `meta.last_bank_cat` |
...
- **⌘K 命令面板**(cmdk):动作(开始复习/开始学习 N/再过一遍/去审核,与今日页按钮同名同义,无「混合」类型)+ 前往(八路由)+ 题目(全库字段包含式过滤,选中落 `?qid=` 详情)+ 按 JD 生成。
