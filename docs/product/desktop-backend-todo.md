...
| B6 | **多份简历** | **已完成(2026-09-14,前端幂等 DDL 垫住)**:新表 `resumes(id, name, content, updated_at)`,启动迁移把存量 profile.resume 搬入;简历页 Select 切换 + 新建/重命名/删除(切份 dirty 先确认);「按 JD 生成 · 结合简历」默认取最近编辑,多于一份时出简历选择器;备份信封含 resumes(旧备份无该键按空表导入)。剩余:Rust 迁移注册表正式化,与 B1/B4 一并 |
