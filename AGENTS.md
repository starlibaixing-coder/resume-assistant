...
- **桌面壳**(`src/components/app-shell.tsx`):常驻侧栏导航 + 内容区,窄窗(<lg)侧栏收成图标栏。主题切换在设置页「外观」;清空进度/笔记在设置页「数据管理」(按分类);后退只用于下钻子页(刷题/浏览页左上角"← 回到{分类名}题库",固定回队列,不依赖历史;侧栏同级切换不显示后退;Cmd+← 为系统级 history.back);浏览页 = 管理查阅视图(手风琴式:模块纵向堆叠带统计/进度条,题目行内展开;答案普通折叠,无笔记面板——答题/评分/笔记都在刷题页);生题数量由 LLM 按知识点广度自定(prompt 有上限约束),不设数量选择。窗口默认 1200×800(`src-tauri/tauri.conf.json`)。
