...
- **UI 组件照 shadcn 官方用法**:不写原生标签,不自创结构变体;缺组件按官方方式补源码进 `src/components/ui/`;有公共组件(PageHeader、SectionHead、EmptyState、ErrorState)就复用,不各写各的。**设计 v6「墨笺 Ink Study」(2026-09-04,`specs/2026-09-04-ui-redesign.md` §v6,默认深色身份)**:墨绿近黑底/暖米字/烛光琥珀主色,Fraunces Variable 只用于数字与大字(中文走系统黑体,经 @fontsource-variable/fraunces 离线打包),氛围光+纸纹+进场 stagger(尊重 reduced-motion);其上承载 **v4「今日驱动」IA**:桌面壳(源列表侧栏 Cmd+1..6 / 工具栏 历史‹›+⌘K 面板 `data-app-nav` / 状态栏)+ 四空间导航(今日 `/` 计划清单与「开始练习」主 CTA;练习 `/session` **跨全库混排**,进度按 (分类,题id) 落库;题库 `/library` 分类切换+浏览/审核 Tabs;求职 /profile),沉浸守卫认 `/session`。旧路由 /drafts、/my、/:category[/:quiz|/browse] 全部重定向兼容,勿删。表单 Label+必填星号,多选一用 RadioGroup,错误用 Alert,**不可逆操作一律 AlertDialog**;行内动作用 ghost icon + ContextMenu。学习页吸底操作条按 `--page-pad-y`(壳层页容器 flex 定义)对齐,改壳层内边距必须联动;main 恒常渲染,沉浸切换只藏 chrome。**v5 功能深化(2026-09-04)**:会话内「不会」即时重排(重练副本放行才出队);评分反馈与逐题小结带具体复习日期;今日页复习预测(7 天到期分桶)/连续天数;⌘K 题目直搜(`/library?category=X&qid=` 深链选中详情);题库行便签标 + 详情笔记预览 + 来源/关键词筛选。搜索事件必须挂 ⌘K 对话框的 DOM onInput(cmdk 根的 value/onValueChange 是选中项语义,不是搜索文本,勿再踩)。 **v7 布局修正(2026-09-04)**:JD 管理=双列卡片栅格(卡内琥珀「按 JD 生成题目」主按钮);简历管理=编辑器版式(标题栏含字数/保存/只存本机);添加题目=三张模式卡选择、一次一条流程(`data-add-mode` 定位,`?jd=` 深链自动切模式);列表一律用圆角卡片容器 + divide-y,禁止全宽发丝线行。
