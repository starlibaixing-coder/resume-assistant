...
2. **UI 决策先查 skill**:做界面设计或使用组件前,必须先调用相关 skill(如 `ui-ux-pro-max` 查 UX 准则/反模式)验证做法,不凭通用惯例或直觉拍板;skill 结论与仓库现有约定冲突时以仓库约定为准并说明。(教训:行内文本按钮、展开抽屉装动作、文字冒充图标,都是没查 skill 直接动手的产物。)
