...
- **UI 组件照 shadcn 官方用法**:不写原生标签,不自创结构变体;缺组件按官方方式补源码进 `src/components/ui/`;有公共组件(PageHeader、SectionHead、EmptyState、ErrorState、TwoPane)就复用,不各写各的。**设计 v8「暖纸书页」(2026-09-07,`specs/2026-09-04-ui-redesign.md` §v8,用户从四方向中选定,默认浅色纸面)**:米纸底/墨色字/朱砂点缀,宋体显示字体(`--font-display`)用于题目大标题与页头;深色为「夜读」暖黑纸伴生;标签药丸形;答案列宽 ≤38rem 保阅读行长;其上承载 **v4「今日驱动」IA**:桌面壳(源列表侧栏 / 工具栏 历史‹›+⌘K 面板 `data-app-nav` / 状态栏)+ 四空间导航(今日 `/` 交接式排程;练习 `/session` **跨全库混排**,进度按 (分类,题id) 落库;题库 `/library` 分类切换+浏览/审核 Tabs;求职 /profile),沉浸守卫认 `/session`。旧路由 /drafts、/my、/:category[/:quiz|/browse] 全部重定向兼容,勿删(站内链接一律直连新路由,不再绕重定向)。表单 Label+必填星号,多选一用 RadioGroup,错误用 Alert,**不可逆操作一律 AlertDialog**。学习页吸底操作条按 `--page-pad-y`(壳层页容器 flex 定义)对齐,改壳层内边距必须联动;main 恒常渲染,沉浸切换只藏 chrome。**v5 功能深化(2026-09-04)**:会话内「不会」即时重排(重练副本放行才出队);评分反馈与逐题小结带具体复习日期;今日页复习预测(7 天到期分桶)/连续天数;⌘K 题目直搜(`/library?category=X&qid=` 深链选中详情);题库行便签标 + 详情笔记预览 + 来源/关键词筛选。搜索事件必须挂 ⌘K 对话框的 DOM onInput(cmdk 根的 value/onValueChange 是选中项语义,不是搜索文本,勿再踩)。 **v9 视觉与筛选(2026-09-07)**:难度/状态/来源芯片筛选(模块保留下拉)、JD 内联创建卡;全站无边框——层次由面色分层承担(纸底/卡面/加深输入面 + 阴影),按钮 outline 变体仅限特例,链接不加下划线。 **v7 布局修正(2026-09-04)**:简历管理=编辑器版式(标题栏含字数/保存/只存本机);添加题目=三张模式卡选择(`data-add-mode` 定位,`?jd=` 深链自动切模式);列表一律用圆角卡片容器 + divide-y,禁止全宽发丝线行。 **v10 交互模型重做(2026-09-07,本节交互约定以此为准)**:① 管理面统一「选行 → 详情就地操作」双栏工作台(`two-pane.tsx`:题库浏览/待审核/JD 管理;≥lg 右侧 sticky 详情栏,<lg 详情全屏覆盖层带"返回列表");② 编辑一律就地(题目/JD 编辑在详情栏内表单,**编辑弹窗已删**),不可逆确认仍 AlertDialog;③ 今日=交接式排程(第一件未完成事=「现在」卡唯一主 CTA,其余弱化「接下来」,零值不出现);④ 练习评分反馈就地语义色闪现 ~900ms 自动推进(反馈归属当前题,勿再写成跨题残留);`?qid=` 单题直练;⑤ 侧栏:今日/练习/题库/**审核(一级项,带待审数)**/【求职】JD·简历/设置,Cmd+1..7,`Cmd+←/→` 历史键已真实绑定;⑥ 简历等页面 dirty 走全局守卫(`lib/nav-guard.ts`,shell 的 `guardedNavigate` 统一拦 ⌘K/Cmd+数字/历史键,未保存先确认);页面级键盘统一 `lib/use-page-keys.ts`(内置输入框/弹层让键,监听稳定勿再手写 window keydown);⑦ /add 三段常驻挂载 hidden 切换(切模式不丢已填内容)。方法教训:交互范式改动先用**可操作的**多方向 HTML(仓库根 `interaction-directions.html`)让用户挑,静态图评不了交互。
