...
### Q04.35【中·手写】瀑布流布局怎么实现？有哪些方案？

**考察点:** 三种方案：①CSS columns（最简单，但顺序是列优先）②JS 分列（计算每列高度，新元素放最短列，顺序正确）③CSS Grid（masonry 还在实验中）。实际项目多用 JS 分列方案，控制精确、顺序正确。
...
### Q04.36【中·手写】如何实现一个轮播图（carousel）？请手写。

**考察点:** 核心：用 transform: translateX 做位移，定时器自动切换 + 拖拽/点击手动切换；无缝循环靠克隆首尾元素实现。
...
### Q04.37【高·概念】Taro 跨端框架的原理是什么？

**考察点:** Taro 原理是「编译时 + 运行时」：编译时把 React 代码转成各端的代码（小程序转 WXML/WXSS/JS，H5 转原生标签）；运行时提供统一的组件库和 API 适配层，抹平各端差异。本质是「用 React 语法描述 UI，编译器翻译成各端产物」。
...
### Q04.38【高·高频·手写】虚拟滚动（virtual scroll）的实现原理是什么？

**考察点:** 虚拟滚动只渲染可视区域的列表项，用绝对定位 + transform 偏移，滚动时动态替换内容，靠一个大的占位元素撑出总高度。核心：计算可视区间 [startIndex, endIndex]，只渲染这部分，用 padding/transform 撑出滚动条。
...
### Q04.39【高·概念】Electron 和 NW.js（node-webkit）有什么区别？

**考察点:** 两者都用 Chromium + Node 做 Web 桌面应用，区别在架构：Electron 多进程（主进程用 Node + 渲染进程用 Chromium），渲染进程默认隔离、要 IPC 调 Node；NW.js 单进程，把 Node 直接注入 DOM。Electron 进程隔离更安全、生态更主流，NW.js 基本被淘汰。
