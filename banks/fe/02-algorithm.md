...
### Q02.5【初·高频·手写】给定两个已排序数组，将它们合并成一个新的有序数组（或把 nums2 合并进 nums1 末尾的预留空间）。

**考察点:** 双指针从后往前（原地合并）或从前往后（新数组）依次比较填入，O(m+n) 时间。
...
### Q02.6【初·高频·手写】判断一个字符串（或整数）是否是回文。

**考察点:** 字符串：左右双指针向中间靠拢，逐字符比较。整数：转字符串同法，或反转数字后比较（注意溢出用 BigInt）。
...
### Q02.7【初·高频·手写】比较两个版本号 version1 和 version2（形如 `1.0.1`、`1.01`、`7.5.2.4`）。若 v1 > v2 返回 1，< 返回 -1，相等返回 0。

**考察点:** 按 `.` 切分成修订号数组，从左到右逐段比较（当作数字比，忽略前导零），某段短的补 0。
...
### Q02.8【中·高频·手写】实现快速排序，并分析时间复杂度。

**考察点:** 选一个基准（pivot），把小于 pivot 的放左、大于的放右，再对左右两段递归排序。
...
### Q02.9【中·高频·手写】反转一个单链表（迭代与递归两种写法）。

**考察点:** 迭代：用 prev/curr 双指针，逐个把 next 指向前一个节点。递归：先翻转子链表，再把当前节点接到翻转后链表的末尾。
...
### Q02.10【中·高频·手写】判断链表中是否有环。如果有，能否找到环的入口节点？

**考察点:** 判断有环：快慢指针（Floyd 判圈），快指针每次走 2 步、慢指针每次走 1 步，相遇即有环。找入口：相遇后让一个指针回到头，两者同速再走，再次相遇点就是入口。
...
### Q02.11【中·高频·手写】给定一个只包含 `(`、`)`、`{`、`}`、`[`、`]` 的字符串，判断括号是否合法匹配。

**考察点:** 用栈：遇左括号入栈，遇右括号看栈顶是否是对应的左括号，匹配则出栈，最后栈空即合法。
...
### Q02.12【中·高频·手写】在未排序数组中找到第 K 大的元素。

**考察点:** 两种主流解法：①小顶堆维护 size=K，堆顶即第 K 大；②快速选择（quickSelect），平均 O(n)。
...
### Q02.13【中·高频·手写】给定股票每日价格，只能买卖一次，求最大利润（买卖股票的最佳时机）。

**考察点:** 遍历过程中维护「历史最低价」minPrice，每天算「当天卖出能赚多少 = price - minPrice」，取最大值。
...
### Q02.14【中·高频·手写】实现对象的深度优先遍历（DFS）与广度优先遍历（BFS）。

**考察点:** DFS 用递归（或栈）一路向下钻；BFS 用队列一层一层扫。两者都需 visited 集合防环。
...
### Q02.15【中·手写】用两个队列实现一个栈，支持 push / pop / top 操作。

**考察点:** 两种实现：①push 时把新元素加到空队列 q2，再把 q1 全部倒进 q2，交换 q1/q2（保证 q1 队首始终是栈顶）；②pop 时把 q1 前 n-1 个倒进 q2，剩最后一个弹出。
...
### Q02.16【高·高频·手写】归并排序和快速排序有什么区别？

**考察点:** 归并排序是「自顶向下分两半 → 递归排序 → 合并两个有序数组」，稳定且始终 O(n log n)，但需要 O(n) 额外空间。
...
### Q02.17【高·高频·手写】给定 n 个非负整数表示柱子的高度图，计算按此排列能接多少雨水。

**考察点:** 每个位置能接的水 = min(左边最高, 右边最高) - 当前高度。可用动态规划预处理左右最大值，或用左右双指针 O(1) 空间。
...
### Q02.18【高·高频·手写】给定不同面额的硬币 coins 和总金额 amount，求凑成总金额所需的最少硬币数。每种硬币可使用无限次。

**考察点:** 完全背包 DP：dp[i] 表示金额 i 的最少硬币数，dp[i] = min(dp[i], dp[i-coin]+1)，遍历所有 coin。
