...
### Q15.123【中·高频·概念】什么是 CSRF（跨站请求伪造）攻击？它是怎么发生的？

**考察点:** CSRF 是攻击者诱导已登录用户在不知情下，用其登录凭证（cookie 自动携带）向目标网站发起恶意请求。
...
### Q15.124【中·高频·概念】什么是 XSS（跨站脚本）攻击？有哪些类型？

**考察点:** XSS 是攻击者把恶意脚本注入到网页里，在其他用户浏览器执行，从而偷 cookie、篡改页面、盗号。分存储型/反射型/DOM 型。
...
### Q15.125【中·高频·概念】cookie 有哪些重要的安全属性？分别防止什么？

**考察点:** 核心安全属性：HttpOnly（防 JS 读取，挡 XSS 偷 cookie）、Secure（只走 HTTPS，防中间人窃听）、SameSite（防 CSRF）、Domain/Path（缩小作用域）。
...
### Q15.126【高·概念】除了 XSS 和 CSRF，前端还有哪些常见安全威胁？点击劫持怎么防？

**考察点:** 常见还有：点击劫持（用透明 iframe 诱骗点击）、中间人攻击、依赖供应链攻击、开放重定向。点击劫持靠 X-Frame-Options 或 CSP frame-ancestors 禁止页面被嵌套来防御。CSP 是缓解多种威胁的万能基石。
